...
import {
    ...
    FieldType
} from './Formidable/Field/field.abstract';
...
export default class Validator<T extends FormidableForm> {
    ...
    constructor(
        private readonly validationCtor: new (...args: any[]) => T,
        private formData: T
    ) {
    ...
    public setForm(form: T) {
        this.formData = form;
    }

    public async validate(): Promise<boolean> {
        console.log('validating');
        // this.obj = plainToClass(
        //     this.validationCtor,
        ...
        // this.obj = await transformAndValidate(
        this.obj = await plainToClass(
            this.validationCtor,
            this.formData
        );
        this.validationErrors = await validate(this.obj);
        console.log(
            'validated: ',
            JSON.parse(JSON.stringify(this.formData)),
